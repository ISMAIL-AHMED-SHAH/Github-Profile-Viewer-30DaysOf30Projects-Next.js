...
export default function GitHubProfileViewer() {
  ...
  return (
    <div className={`flex flex-col items-center justify-center min-h-screen ${isDarkTheme ? 'bg-gray-900' : 'bg-gray-100'} py-8`}>
      <Card className={`w-full max-w-2xl p-8 rounded-lg shadow-2xl bg-gray-800 border border-gray-700 transition-all hover:border-blue-500 hover:shadow-blue-500/50 ${isDarkTheme ? 'bg-gray-800 text-white' : 'bg-white text-black'}`}>
        <CardHeader className="text-center">
          <CardTitle className="text-4xl font-bold p-0 m-0 bg-clip-text text-transparent bg-gradient-to-r from-purple-500 to-pink-500">
            GitHub Profile Viewer
          </CardTitle>
          <CardDescription className="text-gray-600">
            Search for a GitHub username and view their profile and repositories, search like: ISMAIL-AHMED-SHAH
          </CardDescription>
          {/* Theme toggle button */}
          <Button 
  onClick={toggleTheme} 
  className={`mt-4 px-4 py-2 rounded-md transition-colors ${
    isDarkTheme 
      ? 'bg-blue-700 text-white hover:bg-blue-600' // Dark theme color
      : 'bg-blue-500 text-white hover:bg-blue-400' // Light theme color
  }`}
>
  ...
  className={`flex-1 border rounded-md p-2 ${
    isDarkTheme ? 'text-white bg-gray-800 border-gray-600' : 'text-black bg-white'
  }`}
/>
...
<Button
  ...
  className={`px-4 py-2 rounded-md transition-colors ${
    loading ? 'bg-gray-300 text-gray-500' : 'bg-green-600 text-white hover:bg-green-500'
  }`}
>
